...
Implements the adaptive routing strategy from Solution Manual section 4.1.
Routes queries to three processing paths based on complexity:
- Simple queries (60%): Fast path with direct retrieval and single LLM call
- Medium queries (30%): Standard path with precedent checking
- Complex queries (10%): Agentic path with ReAct reasoning
"""
...
class ProcessingPath(Enum):
    """Processing paths corresponding to complexity levels"""
    FAST = "fast"
    STANDARD = "standard"
    AGENTIC = "agentic"
...
class RouteDecision:
    ...
    Attributes:
        ...
        processing_time_ms: Expected processing time for this path
        cost_estimate: Estimated cost for this path
    """
    path: str
    complexity: str
    confidence: float
...
class QueryRouter:
    """
    Enhanced query router implementing adaptive routing strategy.
    ...
    Uses pattern matching and heuristics (no LLM dependency) for fast classification.
    """
    ...
    def __init__(self):
        ...
        self.simple_patterns = patterns.get('fast', [])
        ...
        # Phase 5 Fix: Changed default from 15 to 30 (was over-routing to agentic path)
        self.word_count_threshold = thresholds.get('word_count_for_agentic', 30)
        self.min_confidence = thresholds.get('min_confidence', 0.6)
        
        # Default fallback
        self.default_path = router_config.get('default_path', 'standard')
        
        # Processing path expectations (from Solution Manual)
        self.path_expectations = {
            ProcessingPath.FAST.value: {
                'latency_target_ms': 1500,  # 0.8-1.5s
                'cost_estimate': 0.02,      # ~$0.02 per query
                'description': 'Simple factual queries with direct retrieval'
            },
            ProcessingPath.STANDARD.value: {
        ...
        logger.info("Enhanced QueryRouter initialized")
        logger.info(f"Simple patterns: {len(self.simple_patterns)}")
        logger.info(f"Standard patterns: {len(self.standard_patterns)}")
        logger.info(f"Agentic patterns: {len(self.agentic_patterns)}")
    ...
    def route(
        ...
        # Map to complexity
        complexity_mapping = {
            ProcessingPath.FAST.value: QueryComplexity.SIMPLE.value,
            ProcessingPath.STANDARD.value: QueryComplexity.MEDIUM.value,
            ProcessingPath.AGENTIC.value: QueryComplexity.COMPLEX.value
        }
        complexity = complexity_mapping.get(path, QueryComplexity.MEDIUM.value)
        
        # Get path expectations
        path_info = self.path_expectations.get(path, {})
        ...
        decision = RouteDecision(
            ...
            reasoning=reasoning,
            features=features,
            processing_time_ms=path_info.get('latency_target_ms', 2500),
            cost_estimate=path_info.get('cost_estimate', 0.06)
        )
        
        logger.info(
            f"Routed to {path} path (complexity: {complexity}, "
    ...
    def _extract_features(
        ...
        # Pattern matches
        features['simple_pattern_matches'] = sum(
            1 for pattern in self.simple_regex if pattern.search(query)
        )
    ...
    def _calculate_path_scores(self, features: Dict[str, Any]) -> Dict[str, float]:
        ...
        """
        scores = {
            ProcessingPath.FAST.value: 0.0,
            ProcessingPath.STANDARD.value: 0.0,
            ProcessingPath.AGENTIC.value: 0.0
        }
        
        # Fast path scoring (simple factual queries)
        if features['simple_pattern_matches'] > 0:
            scores[ProcessingPath.FAST.value] += 0.5
        if features['word_count'] <= 10:
            scores[ProcessingPath.FAST.value] += 0.3
        if features['entity_count'] <= 2:
            scores[ProcessingPath.FAST.value] += 0.2
        
        # Standard path scoring (decisions/recommendations)
        if features['standard_pattern_matches'] > 0:
            scores[ProcessingPath.STANDARD.value] += 0.6  # Strong signal
        if 6 < features['word_count'] <= 20:
            scores[ProcessingPath.STANDARD.value] += 0.2
        if 2 < features['entity_count'] <= 5:
            scores[ProcessingPath.STANDARD.value] += 0.2
        
        # Agentic path scoring (complex analysis)
        if features['agentic_pattern_matches'] > 0:
            scores[ProcessingPath.AGENTIC.value] += 0.7  # Very strong signal
        if features['word_count'] > self.word_count_threshold:
            scores[ProcessingPath.AGENTIC.value] += 0.3
        if features['entity_count'] > self.entity_threshold:
            scores[ProcessingPath.AGENTIC.value] += 0.3
        if features['has_comparison']:
            scores[ProcessingPath.AGENTIC.value] += 0.4
    ...
    def _decide_path(
        ...
        if best_path == ProcessingPath.FAST.value:
            if features['simple_pattern_matches'] > 0:
        ...
        elif best_path == ProcessingPath.STANDARD.value:
            if features['standard_pattern_matches'] > 0:
        ...
        elif best_path == ProcessingPath.AGENTIC.value:
            if features['agentic_pattern_matches'] > 0:
    ...
    def _check_decomposition_indicators(self, query: str, features: Dict[str, Any]) -> bool:
        ...
        try:
            from llm_integration.query_decomposition import QueryComplexityDetector
        except ImportError:
            # Fallback if module not available
            return False
        
        # Use the sophisticated complexity detector from the decomposition module
        try:
            # Create a simple config with default values
            config = {
                "min_query_length": 150,
                "min_entity_count": 4,
                "min_conjunction_count": 2,
                "comparison_keywords": [
            ...
            detector = QueryComplexityDetector(config)
            entities = features.get('entities', [])
            
            # Use the full complexity detection
            analysis = detector.detect_complexity(query, entities)
            
            # Return the decomposition decision
            return analysis['needs_decomposition']
            
        except Exception as e:
            logger.warning(f"Error in advanced decomposition detection, using fallback: {e}")
