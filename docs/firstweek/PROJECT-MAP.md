...
- Public showcase: separate six-project authored collection and anonymous read-only API. Learning RAG is published as completed and self-coded at the owner's direction. The collection must never import the private manifest, database, project documents, company teams, profiles, or saved conversations.
